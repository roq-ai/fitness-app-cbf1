...
type ColumnType = ColumnDef<ExerciseInterface, unknown>;
...
export function ExerciseListPage(props: ExerciseListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getExercises({
        relations: [, 'workout_exercise.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ExerciseInterface>>(
    () => `/exercises?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'calories_burned', header: 'calories_burned', accessorKey: 'calories_burned' },
