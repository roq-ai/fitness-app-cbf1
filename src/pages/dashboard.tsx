...
import { getWorkoutExercises } from 'apiSdk/workout-exercises';
import { WorkoutExerciseInterface } from 'interfaces/workout-exercise';
import { getSharedWorkouts } from 'apiSdk/shared-workouts';
import { SharedWorkoutInterface } from 'interfaces/shared-workout';

import { SharedWorkoutListPage as ShowCaseList } from 'pages/shared-workouts';

function DashboardPage() {
  const { session } = useSession();

  const workoutStat = useSWR<PaginatedInterface<WorkoutInterface>>(
    () => `/workouts`,
    () => getWorkouts({ limit: 1, offset: 0 }),
  );

  const workoutExerciseStat = useSWR<PaginatedInterface<WorkoutExerciseInterface>>(
    () => `/workout-exercises`,
    () => getWorkoutExercises({ limit: 1, offset: 0 }),
  );

  const sharedWorkoutStat = useSWR<PaginatedInterface<SharedWorkoutInterface>>(
    () => `/shared-workouts`,
    () => getSharedWorkouts({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Workouts',
      result: workoutStat?.data?.totalCount,
      isLoading: workoutStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Workout Exercises',
      result: workoutExerciseStat?.data?.totalCount,
      isLoading: workoutExerciseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Shared Workouts',
      result: sharedWorkoutStat?.data?.totalCount,
      isLoading: sharedWorkoutStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Shared Workouts
                </Text>
                <NextLink href={`shared-workouts`} passHref legacyBehavior>
                  <Button
