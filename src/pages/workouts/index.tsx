...
type ColumnType = ColumnDef<WorkoutInterface, unknown>;
...
export function WorkoutListPage(props: WorkoutListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getWorkouts({
        relations: ['user', 'shared_workout.count', 'workout_exercise.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WorkoutInterface>>(
    () => `/workouts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('shared_workout', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'shared_workout',
          header: 'Shared Workout',
          accessorKey: 'shared_workout',
          cell: ({ row: { original: record } }: any) => record?._count?.shared_workout,
        }
      : null,
    hasAccess('workout_exercise', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'workout_exercise',
          header: 'Workout Exercise',
          accessorKey: 'workout_exercise',
          cell: ({ row: { original: record } }: any) => record?._count?.workout_exercise,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/workouts/view/${record.id}`} passHref legacyBehavior>
                <Button
